...
document.getElementById('fileInput').addEventListener('change', async (event) => {
    ...
    if (files.length === 1) {
        ...
        tabla1mostrarTablaSolicitudesRepetidas(quejaData, "Ordenes Repetidas - Queja");
    }
});
...
async function processFile(content) {
    ...
    rows.forEach((row) => {
        ...
        if (cells.length >= 19) {
            tipo = cells[2]?.textContent.trim().toUpperCase().startsWith("C") 
                ? cells[2].textContent.trim().charAt(0) + cells[2].textContent.trim().slice(1, 3) 
                : cells[2]?.textContent.trim().charAt(0) + cells[2].textContent.trim().slice(1, 3);

            dataPoints.push({
                num: cells[0]?.textContent.trim() || '',          // "Número"
                order: cells[1]?.textContent.trim() || '',        // "Solicitud"
                tipo: tipo || '',                                 // "Tipo"
                h: cells[3]?.textContent.trim() || '',            // "H"
                ta: cells[4]?.textContent.trim() || '',           // "Ta"
                cto: cells[5]?.textContent.trim() || '',          // "Cto"
                meter: cells[6]?.textContent.trim() || '',        // "Medidor(es)"
                rpu: cells[9]?.textContent.trim() || '',          // "RPU"
                origen: cells[10]?.textContent.trim() || '',                          // "Origen"
                name: cells[11]?.textContent.trim() || '',         // "Nombre"
                address: cells[12]?.textContent.trim() || '',     // "Dirección"
                colony: cells[13]?.textContent.trim() || '',                             // "Colonia"
                tipoRed: cells[14]?.textContent.trim() || '',     // "TipoRed"
                registro: cells[15]?.textContent.trim() || '',    // "Registro"
                date: cells[16]?.textContent.trim() || '',        // "Fecha Reg"
                observaciones: cells[17]?.textContent.trim() || ''// "Observaciones"
            });
        }
    });
...
function determineRequestType(data) {
    ...
    if (!tipo) return "SolicitudTerminada"; // Valor predeterminado si no hay tipos
    return tipo.startsWith("Q") ? "SolicitudTerminadaQueja" : "SolicitudTerminadaContrato";
}
